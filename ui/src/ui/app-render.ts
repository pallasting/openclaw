...
import { renderAgents, resolveConfiguredModels } from "./views/agents.ts";
          ...
          state.tab === "chat"
            ? renderChat({
                ...
                onModelChange: (model: string) =>
                  state.handleSessionsPatch(state.sessionKey, { model: model || null }),
                reasoningLevel:
                  state.sessionsResult?.sessions?.find((s) => s.key === state.sessionKey)
                    ?.reasoningLevel ?? "off",
                onReasoningLevelChange: (level: any) =>
                  state.handleSessionsPatch(state.sessionKey, { reasoningLevel: level }),
